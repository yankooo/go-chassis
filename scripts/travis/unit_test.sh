...
# For transport test
mkdir -p $GOPATH/src//core/transport/tls
mkdir -p $GOPATH/src//examples/discovery/server/log
mkdir -p $GOPATH/src//examples/discovery/server/conf
...
for d in $(go list ./... | grep -v vendor |  grep -v third_party | grep -v examples); do
    ...
    if [ $(ls | grep _test.go | wc -l) -gt 0 ]; then
        ...
        if [ -f coverage.out ]; then
            sed '1d;$d' coverage.out >> $GOPATH/src//coverage.txt
        fi
    fi
done
